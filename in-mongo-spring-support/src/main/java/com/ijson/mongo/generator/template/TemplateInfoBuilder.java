 ...
 */
public class TemplateInfoBuilder implements TemplateHanlder {
    ...
    private void createdInfo(String prefix, List<ObjectInfo> objectInfos, GenConfig config) {
        ...
        String daoPath = config.getGenPath() + "/" + projectName + "/" + prefix + "java/" + config.getPackager().replace(".", "/") + "/dao/entity/info/";
        ...
        if (!CollectionUtils.isEmpty(objectInfos)) {
            for (ObjectInfo objectInfo : objectInfos) {
                ...
                FileOperate.getInstance().newCreateFile(
                        daoPath + objectInfo.getSimpleName() + "Info.java",
                        TemplateUtil.getTemplate("info.templete",maps)

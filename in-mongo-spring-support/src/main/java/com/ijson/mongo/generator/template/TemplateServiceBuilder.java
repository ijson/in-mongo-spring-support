 ...
 */
public class TemplateServiceBuilder implements TemplateHanlder {
    ...
    private void createdInfo(String prefix, List<ObjectInfo> objectInfos, GenConfig config) {
        ...
        String daoPath = config.getGenPath() + "/" + projectName + "/" + prefix + "java/" + config.getPackager().replace(".", "/") + "/service/";
        ...
        if (!CollectionUtils.isEmpty(objectInfos)) {
            for (ObjectInfo objectInfo : objectInfos) {
                ...
                FileOperate.getInstance().newCreateFile(
                        daoPath + objectInfo.getSimpleName() + "Service.java",
                        TemplateUtil.getTemplate("service.templete",maps)

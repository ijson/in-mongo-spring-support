...
public class TemplateServiceImplBuilder implements TemplateHanlder {
    ...
    private void createdInfo(String prefix, List<ObjectInfo> objectInfos, GenConfig config) {
        ...
        String daoPath = config.getGenPath() + "/" + projectName + "/" + prefix + "java/" + config.getPackager().replace(".", "/") + "/service/impl/";
        ...
        if (!CollectionUtils.isEmpty(objectInfos)) {
            for (ObjectInfo objectInfo : objectInfos) {
                ...
                FileOperate.getInstance().newCreateFile(
                        daoPath + objectInfo.getSimpleName() + "ServiceImpl.java",
                        TemplateUtil.getTemplate("service_impl.templete",maps)

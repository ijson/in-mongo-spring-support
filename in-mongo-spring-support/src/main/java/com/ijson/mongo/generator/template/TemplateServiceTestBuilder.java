 ...
 */
public class TemplateServiceTestBuilder implements TemplateHanlder {
    ...
    private void createdInfo(String prefix, List<ObjectInfo> objectInfos, GenConfig config) {
        ...
        String daoPath = config.getGenPath() + "/" + projectName + "/" + prefix + "java/" + config.getPackager().replace(".", "/") + "/service/test/";
        ...
        if (!CollectionUtils.isEmpty(objectInfos)) {
            for (ObjectInfo objectInfo : objectInfos) {
                ...
                FileOperate.getInstance().newCreateFile(
                        daoPath + objectInfo.getSimpleName() + "ServiceTest.java",
                        TemplateUtil.getTemplate("service_test.templete",maps)
